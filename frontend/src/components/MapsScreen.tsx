      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: "#888" }}>
            {mode === "active" ? `${pct}% compatible` : "Someone nearby"}
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: "#999" }}>
            {peer.similarity ? Math.round(peer.similarity * 100) + "% compatible" : ""}
          </div>
        </div>
    ...
    <div style={{
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.timestamp + m.fromId} style={{
              ...
              <div style={{
                maxWidth: "80%", padding: "6px 9px", borderRadius: 12,
      ...
      <form onSubmit={(e) => {
        ...
        <input value={draft} onChange={(e) => setDraft(e.target.value)}
          ...
          style={{
            flex: 1, borderRadius: 999, border: "1px solid rgba(0,0,0,0.12)",
            padding: "6px 10px", fontSize: 12, outline: "none",
          }}
        />
        <button type="submit" style={{
          borderRadius: 999, border: "none", padding: "6px 10px",
          fontSize: 11, fontWeight: 600, background: "#C0392B", color: "#fff",
...
export default function MapScreen({ onChatRequest }: { onChatRequest?: (uid: string) => void }) {
  ...
  // ── Single shared socket from hook ──
  ...
  // ── Chat listeners on shared socket ──
  useEffect(() => {
    ...
    const onIncoming = (payload: { fromId: string; preview?: string }) => {
      setPendingRequests(prev => ({
        ...prev,
        [payload.fromId]: {
          fromId: payload.fromId, toId: myId,
          preview: payload.preview ?? "", timestamp: Date.now(),
        },
      }));
  ...
  // Center map when real position arrives
  useEffect(() => {
  ...
  // Clear halo when selectedUser is nulled
  useEffect(() => {
    ...
    <div style={{
        ...
        <ProfilePopup
          ...
          onAccept={() => {
            ...
            socket.emit("chat:accept", {
              fromId: req.fromId, toId: myId,
              conversationId: cid, peerName: "You",
            });
            const convo: AcceptedConvo = {
              conversationId: cid, peerId: req.fromId,
            ...
            setPendingRequests(prev => {
              const n = { ...prev }; delete n[req.fromId]; return n;
            });
            setSelectedUser(null);
          ...
          onDecline={() => {
            if (!selectedUser) return;
            setPendingRequests(prev => {
              const n = { ...prev }; delete n[selectedUser.uid]; return n;
            });
            setSelectedUser(null);
          }}
          onGoToChat={() => {
        ...
        <ChatOverlay
          ...
          onSend={(text) => {
            ...
            const payload: ChatMessage = {
              conversationId: cid, fromId: myId,
              toId: chatOverlayPeer.uid, text, timestamp: Date.now(),
            };
            setChatOverlayCid(cid);
            setConversations(prev => ({
              ...prev, [cid]: [...(prev[cid] ?? []), payload],
            }));
            socket.emit("chat:message", payload);
            socket.emit("chat:request", {
              fromId: myId, toId: chatOverlayPeer.uid, preview: text,
            });
      ...
      <ChatPanel
        ...
        onSend={(text) => {
          ...
          const payload: ChatMessage = {
            conversationId: activeChatConvo.conversationId,
            fromId: myId, toId: activeChatConvo.peerUid,
            text, timestamp: Date.now(),
          };
          setConversations(prev => ({
            ...prev,
            [activeChatConvo.conversationId]: [
              ...(prev[activeChatConvo.conversationId] ?? []),
              payload,
            ],
          }));
          socket.emit("chat:message", payload);
